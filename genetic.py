...
class Poblacion:
    ...
    def evaluar_poblacion(self, funcion_objetivo, optimizacion, verbose=True):
        """
    ...
    def optimizar(self, funcion_objetivo, optimizacion, n_generaciones = 50,
        ...
        self.resultados_df = pd.DataFrame(
            ...
            "historia"             : self.historico_individuos
            }
        )
...
datos = pd.read_csv(r'article_data.txt', delim_whitespace=True)
s = datos["sigma"]**2
covariance = pd.DataFrame(np.diag(s), index=s.index, columns=s.index)

def theory(z, H0, w, x, y):
    return H0*(1 + z*(w + x*z - y*z**3)**2)


def funcion_objetivo(H0, w, x, y):

    delta = theory(datos["z"], H0,  w, x, y) - datos["H(z)"]
